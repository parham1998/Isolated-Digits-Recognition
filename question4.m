...
for i = 0:9
    ...
    %v_melcepst: MFCC
    test_frames = v_melcepst(s, fs, 'Ed', 12, floor(3*log(fs)), w_size, w_size/3);
    for j = 0:9
        s = audioread(['train/' num2str(j) '_train.wav']);
        train_frames = v_melcepst(s, fs, 'Ed', 12, floor(3*log(fs)), w_size, w_size/3);
...
function d = cepstral_distance(s1, s2, min_s1_s2)    
    d = 0;
    for i = 1:min_s1_s2
    	d = d + sum(abs(s1(i, 1:26) - s2(i, 1:26)));
    end
end